      ...
      <div class="max-w-5xl mx-auto"> 
        ...
        <div class="w-full bg-white overflow-y-auto max-h-75">
            <div class="shadow" style="">
              <h1 class="py-4 px-10 border-b font-bold text-gray-700 rounded-t bg-white">Edit Employee</h1>
              <div class="min-w-full rounded-b bg-gray-100 px-10 pt-5 pb-8 border-t border-white">
                  <div class="flex">
                      <div class="w-2/4">
                          <div class="w-full mb-6">
                              <p class="mb-2 text-gray-600">Name</p>
                              <input type="text" placeholder="" class="block w-full bg-white text-gray-700 border border-gray-400 rounded py-2 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 capitalize" v-model="name">
                          </div>
                          <div class="w-full mb-6">
                          ...
                          <div class="w-full mb-6">
                              ...
                              <label class="mr-2 inline-flex items-baseline">
                                  <input type="radio" name="gender" value="male" v-model="gender" :checked="gender == 'male'"><span class="ml-1">Male</span></label>
                          ...
                          <div class="w-full mb-6">
                            ...
                            <div class="flex">
                                ...
                                <div class="bg-cover w-3/5" style="height: 120px; width: 120px" :style="{ 'background-image': 'url(' + checkImgShow(photo) + ')' }"></div>
                            </div>
                          </div>
                      </div>
                          ...
                          <div class="w-full mb-6">
                              ...
                              <div class="relative">
                                <select class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow-inner leading-tight focus:outline-none focus:shadow-outline capitalize" v-model="role_jobs">
                                  <option v-for="(item, idx) in roleJob" :key="idx" :value="item.id" class="capitalize" :selected="role_jobs == item.id"> {{ item.divition }} - {{ item.position }} </option>
                                </select>
                          ...
                          <div class="w-full mb-6">
                              ...
                              <div class="relative">
                                <select class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow-inner leading-tight focus:outline-none focus:shadow-outline capitalize" v-model="status_employee">
                                  <option v-for="(item, idx) in employeeStatus" :key="idx" :value="item.id" class="capitalize" :selected="status_employee == item.id"> {{ item.status }} </option>
                                </select>
                                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="fill-current h-4 w-4"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"></path></svg></div>
                              </div>
                          </div>
                          <div class="w-full mb-6">
                              <p class="mb-2 text-gray-600">Password</p>
                              <input type="input" placeholder="" class="block w-full bg-white text-gray-700 border border-gray-400 rounded py-2 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" v-model="password">
                          </div>
                      </div>
                  </div>
              </div>
              <transition name="fade">
                <div class="custom-modal-succes capitalize absolute flex justify-center items-center text-white bg-green-400 rounded" v-if="addSuccess">Success edit Employee</div>
              </transition>
              <transition name="fade">
              ...
              <transition name="fade">
                <div class="custom-modal-succes capitalize absolute flex justify-center items-center text-white bg-green-400 rounded" v-if="addDeleteSucces">Success delete Employee</div>
              </transition>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods:{
    ...
    checkImgShow(item){
      return item ? item : '/src/assets/avatar.jpg';
    },
    initEditVal(){
      ...
      this.role_jobs = this.getTheUser.role_job
      this.status_employee = this.getTheUser.status_employee
      this.photo = this.getTheUser.photo ? this.getTheUser.photo : '/src/assets/avatar.jpg'
      this.password = this.getTheUser.password
    },
    save(){
      let get_user = {
          "id" : this.id,
      ...
      if(this.name != '' && this.email != '' && this.phone != '' && this.birth_place != '' && this.photo != '' && this.password != ''){
          this.addSuccess = true;
          this.editedEmployee(get_user);
      }else{
          this.addError = true;
      }
    },
  ...
  watch:{
    ...
    addDeleteSucces(){
        if(this.addDeleteSucces){
            let self = this;
            setTimeout(function(){
                self.addDeleteSucces = false;
                self.$router.push({ name: 'employees' });
            },1500);
        }
    }
  ...
  created() {
      ...
      this.initEditVal();
      if(!this.getTheUser) this.$router.push({ name: 'employees' });
  },
}
</script>